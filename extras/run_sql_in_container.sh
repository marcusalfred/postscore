...
SELECT id, name, email, is_super, handicap, ghin_number FROM players WHERE email = 'admin@example.com';
EOF

# Copy the SQL file to the container
podman cp /tmp/query.sql postscore_db:/tmp/query.sql

# Execute the SQL in the container
podman exec -it postscore_db psql -U postgres -d postscore -f /tmp/query.sql
